...
local originalGetVellumItemString = Scanner.GetVellumItemString

---Returns the canonical WotLK vellum for a craft.
...
function Scanner.GetVellumItemString(craftString)
	...
	if not ClientInfo.HasFeature(ClientInfo.FEATURES.C_TRADE_SKILL_UI) then
		-- Classic craft strings are hashes. Resolve the live tradeskill index and
		-- read the real enchant spell ID from the enchant result link instead of
		-- trying to use the hash as a spell ID.
