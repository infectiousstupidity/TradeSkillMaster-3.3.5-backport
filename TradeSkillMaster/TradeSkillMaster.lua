...
function TSM.OnInitialize(settingsDB)
	...
	-- DBMarket is a long-standing public source used throughout Auctioning operations.
	-- On this local-only 3.3.5 backport, preserve that contract while the rolling
	-- market history is still being established: prefer trusted DBMarket history,
	-- then fall back to the latest scan's robust market value (never DBMinBuyout).
	local function GetAuctionDBMarketValue(itemString)
		return TSM.AuctionDB.GetMarketOrRecentValue(itemString)
	end
	CustomString.RegisterSource("AuctionDB", "DBMarket", L["AuctionDB - Market Value"], GetAuctionDBMarketValue, CustomString.SOURCE_TYPE.PRICE_DB)
	CustomString.RegisterSource("AuctionDB", "DBMarketRecent", L["AuctionDB - Market Value or Recent Value"], GetAuctionDBMarketValue, CustomString.SOURCE_TYPE.PRICE_DB)
