...
L["After an item has expired this many times, it will no longer be posted on the AH. Set this to 0 to disable it."] = "После указанного числа истечений лот больше не выставляется на аукцион. Установите 0, чтобы отключить."
...
L["How TSM determines the market value for the purpose of displaying in the 'Sell' tab of the Vendor UI."] = "Как TSM определяет рыночную стоимость для отображения во вкладке 'Продажа' окна торговца."
L["How long auctions are listed for on the AH."] = "Как долго лоты висят на аукционе."
L["How long in minutes to wait between automatically attempting to re-send items according to your Mailing operations."] = "Сколько минут ждать между автоматическими повторными отправками предметов согласно почтовым операциям."
L["Hr"] = "Час"
L["Hrs"] = "Часов"
L["If eanbled, the gold amount in the top-right of TSM windows will be your total gold, rather than the gold on the current character."] = "Если включено, количество золота в правом верхнем углу окон TSM будет общим по всем персонажам, а не только текущего."
L["If enabled, TSM will add chat messages describing mail being opened."] = "Если включено, TSM будет писать в чат сообщения об открываемой почте."
L["If enabled, TSM will add chat messages describing mail being sent."] = "Если включено, TSM будет писать в чат сообщения об отправляемой почте."
L["If enabled, TSM will automatically track trades where a single type of item is exchanged for an amount of gold as a sale or purchase."] = "Если включено, TSM будет автоматически учитывать обмены одного типа предмета на золото как продажу или покупку."
L["If enabled, TSM will cancel undercut auctions even if they have been bid on."] = "Если включено, TSM будет снимать перебитые лоты, даже если на них уже сделана ставка."
L["If enabled, TSM will display an additional confirmation when attempting to buy an auction above the value set in the 'Buyout alert source' setting. This can help avoid accidental purchases of expensive auctions."] = "Если включено, TSM запросит дополнительное подтверждение при покупке лота дороже значения из настройки 'Источник предупреждения о выкупе'. Это помогает избежать случайных дорогих покупок."
L["If enabled, TSM will load data (i.e. inventory / Accounting / gold tracking) from every realm you have characters on, instead of just connected realms."] = "Если включено, TSM будет загружать данные (инвентарь / Accounting / золото) со всех игровых миров, где есть ваши персонажи, а не только с объединённых."
L["If enabled, auctions above the defined max price will be shown in shopping results."] = "Если включено, лоты дороже заданной максимальной цены будут показаны в результатах поиска."
L["If enabled, items in the selected group will be purchased from the targeted vendor."] = "Если включено, предметы из выбранной группы будут покупаться у торговца."
L["If enabled, items in the selected group will be sold to the targeted vendor."] = "Если включено, предметы из выбранной группы будут продаваться торговцу."
L["If enabled, items that are soulbound will be sold to the targeted vendor."] = "Если включено, персональные предметы будут продаваться торговцу."
L["If enabled, items would need to satisfy a minimum profit before being added to your crafting queue."] = "Если включено, предметы будут добавляться в очередь крафта только при достижении минимальной прибыли."
L["If enables, the total amount of money earned from selling items to the vendor will be displayed in chat."] = "Если включено, общая сумма, полученная от продажи предметов торговцу, будет показана в чате."
L["If this option is enabled any operation created will be stored by TSM without any association to a specific profile that is loaded at the time of creation. This means operations will be available to all profiles. Note: Storing operations globally will also mean no operations are included when sending a profile to a synced account."] = "Если включено, создаваемые операции будут храниться без привязки к текущему профилю и станут доступны во всех профилях. Примечание: при глобальном хранении операции не включаются в профиль при отправке на синхронизируемый аккаунт."
...
L["Import Item IDs from Wowhead"] = "Импорт ID предметов с Wowhead"
...
L["Include soulbound items in the list of items to disenchant."] = "Включать персональные предметы в список для распыления."
...
L["Item/Group is invalid (see chat)."] = "Предмет/группа недействительны (см. чат)."
L["Items"] = "Предметы"
...
L["Loads AuctionDB data for an additional realm for display in the tooltip."] = "Загружает данные AuctionDB дополнительного игрового мира для отображения в подсказке."
...
L["Mailing, Vendoring, and Warehousing operation settings now fully support using custom strings."] = "Настройки операций почты, торговца и склада теперь полностью поддерживают пользовательские строки цен."
...
L["Only the last 6 months of Accounting purchases and sales data for %s was loaded. Consider clearing old Accounting data from the TSM settings on that realm."] = "Данные учёта покупок и продаж для %s загружены только за последние 6 месяцев. Очистите старые данные учёта в настройках TSM на этом игровом мире."
...
L["TSM detected that you just traded %s to %s in return for %s. Would you like Accounting to store a record of this trade?"] = "TSM заметил, что вы отдали %s игроку %s и получили %s. Записать этот обмен в учёт?"
...
L["TSM will display an additional confirmation when attempting to buy an auction above this value. This can help avoid accidental purchases of expensive auctions."] = "TSM запросит дополнительное подтверждение при покупке лота дороже этого значения. Это помогает избежать случайных дорогих покупок."
...
L["The player \"%s\" is already on your whitelist."] = "Игрок \"%s\" уже в вашем белом списке."
...
L["There is no Crafting operation applied to this item's TSM group (%s)."] = "К группе TSM этого предмета (%s) не применена операция крафта."
...
L["You do not need to add \"%s\", alts are whitelisted automatically."] = "Не нужно добавлять \"%s\" — твинки попадают в белый список автоматически."
...
L["ilvl"] = "ур. предмета"
-- 3.3.5 locale fix: keys that were previously hardcoded English literals in the UI
-- (buy/cancel buttons, Back button, Scan tab) and had no locale entries at all
L["Bid"] = "Ставка"
L["Back"] = "Назад"
-- 3.3.5 backport: Full Scan page (new strings that don't exist upstream)
L["Full Auction House Scan"] = "Полное сканирование аукциона"
L["Scan the entire Auction House and save its prices to your local AuctionDB.\n\n|cffffd839%s|r  -  single getAll request, reads everything at once, no cooldown.\n|cffffd839%s|r  -  page by page, slower but no cooldown."] = "Сканирует весь аукцион и сохраняет цены в локальную базу AuctionDB.\n\n|cffffd839%s|r  -  один запрос getAll, читает всё сразу, без задержки.\n|cffffd839%s|r  -  постранично, медленнее, но без задержки."
L["Fast Scan"] = "Быстрое сканирование"
L["Slow Scan"] = "Медленное сканирование"
L["Abort"] = "Прервать"
L["Idle"] = "Ожидание"
L["Open the Auction House, then run Fast or Slow Scan."] = "Откройте аукцион, затем запустите быстрое или медленное сканирование."
L["Scan already in progress. Use Abort to stop it."] = "Сканирование уже идёт. Нажмите «Прервать», чтобы остановить."
L["Previous scan still cleaning up, wait a moment."] = "Предыдущее сканирование ещё завершается, подождите."
L["Open the Auction House first."] = "Сначала откройте аукцион."
L["Cannot query auction right now (throttled)."] = "Сейчас нельзя запросить аукцион (ограничение сервера)."
L["No active scan to abort."] = "Нет активного сканирования для прерывания."
L["Scan aborted by user."] = "Сканирование прервано пользователем."
L["Server throttle never cleared - aborting scan."] = "Ограничение сервера не снялось — сканирование прервано."
L["Fast Scan: waiting for server (getAll)..."] = "Быстрое сканирование: ожидание сервера (getAll)..."
L["Fast Scan returned no data or timed out. Falling back to Slow Scan."] = "Быстрое сканирование не вернуло данных или истекло время. Переход на медленное сканирование."
L["Fast Scan returned no data. Falling back to Slow Scan."] = "Быстрое сканирование не вернуло данных. Переход на медленное сканирование."
L["Processing %d auctions..."] = "Обработка %d лотов..."
L["Paged scan: page %d..."] = "Постраничное сканирование: страница %d..."
L["Page"] = "Страница"
L["Processing"] = "Обработка"
L["Page %d timed out after %ds - aborting."] = "Страница %d не ответила за %d сек. — прерывание."
L["Page %d/%d resolving %d links (pass %d/%d)..."] = "Страница %d/%d: получение %d ссылок (проход %d/%d)..."
L["Skipped %d auctions after %d retries (item cache)."] = "Пропущено %d лотов после %d повторов (кэш предметов)."
L["Retry pass %d/%d (%d items)..."] = "Повторный проход %d/%d (%d предметов)..."
L["Done! %d auctions, %d items in %.1fs (%s mode)"] = "Готово! %d лотов, %d предметов за %.1f сек. (режим %s)"
L["Full scan complete: %d auctions, %d items saved to local AuctionDB (%.1fs)"] = "Полное сканирование завершено: %d лотов, %d предметов сохранено в AuctionDB (%.1f сек.)"
